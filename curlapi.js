...
getTournament().then(tournament=>{tournamentName = tournament.title
...
fetch(compURL+gamesEP+"/"+gameID).then(function (response) {
	if (response.ok) {
...
let teams =	Promise.all([
		fetch(compURL+teamEP+"/"+ game.game_positions[0].competition_team_id).then(response => response.json()),
		fetch(compURL+teamEP+"/"+ + game.game_positions[1].competition_team_id).then(response => response.json()),
	]);
return teams;
...
getGames().then( games => {
	...
	body: JSON.stringify({date:game.draw.starts_at,ccUUID:game.id,gameID:game.name,ccIDteam1:game.game_positions[0].competition_team_id,ccIDteam2:game.game_positions[1].competition_team_id,tournament:tournamentName,gender:gender,draw:game.draw.label})
  }
  ) 
  })
